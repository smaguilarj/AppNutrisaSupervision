...
class VisitaLargaActivity : AppCompatActivity(), AdapterView.OnItemSelectedListener {
                        ...
                        firebaseData.region?.let { item ->
                            ...
                            when (item) {
                                ...
                                "NORTE GOLFO" -> {
                                    ...
                                    Toast.makeText(applicationContext, "no has seleccionado un distrito", Toast.LENGTH_SHORT).show()
                                }
                            }
                        }
                        firebaseData.distrito?.let { distrito ->
                            ...
                            when (distrito) {
                                ...
                                "GUADALAJARA 1" -> {
                                    firebaseData.Nombre_tienda?.let { item -> listGuadalajara1.add(item)
                                    }
                                }
                                "GUADALAJARA 2" -> {
                                    firebaseData.Nombre_tienda?.let { item ->
    ...
    override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, p3: Long) {
       // val result= parent?.getItemAtPosition(position).toString()
        val result = parent?.selectedItem.toString()
        when (result) {
            "CENTRO NORTE" -> {
    ...
    private fun getDistrito(listDistrito: java.util.ArrayList<String>) {
        ...
        val array_adapter_distrito =
            ArrayAdapter(this, android.R.layout.simple_spinner_item, listDistrito)
    ...
    private fun getShop(listTienda: ArrayList<String>) {
       ...
       //spinner_tiendaL.onItemSelectedListener = this
        spinner_tiendaL.onItemSelectedListener=object : AdapterView.OnItemSelectedListener {
